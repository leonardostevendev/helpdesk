...
class database extends PDO {
	...
	function __construct($bt_db_host, $bt_db_name, $bt_db_user, $bt_db_pass, $bt_db_type = 'mysql', $bt_db_charset = 'UTF8', $bt_db_port = 3306) {
		...
		switch ($bt_db_type) {
			...
			case 'odbc':
				if (array_search('odbc', parent::getAvailableDrivers()) === FALSE) {
					$error->create(array('type' => 'sql_type_unsupported', 'message' => 'Unable to find the PDO ODBC database driver, which is required with the current database settings.'));
				}
						
				try {
					$connection = parent::__construct('odbc:Driver={SQL Server};Server='.$bt_db_host.';Database='.$bt_db_name.'; Uid='.$bt_db_user.';Pwd=' . $bt_db_pass);
				}
				catch (\PDOException $e) {
					$error->create(array('type' => 'sql_connect_error', 'message' => $e->getMessage()));
				}
			...
			case 'freetds':
				...
				try {
					$connection = parent::__construct('odbc:Driver=FreeTDS;Server='.$bt_db_host.';Database='.$bt_db_name.'; Uid='.$bt_db_user.';Pwd=' . $bt_db_pass);
				}
				catch (\PDOException $e) {
					$error->create(array('type' => 'sql_connect_error', 'message' => $e->getMessage()));
				}
