...
class mailer {
	...
	public function send_email($array) {
		...
		try {
			...
			$larray['event_severity'] = 'notice';
			$larray['event_number'] = E_USER_NOTICE;
			
			if (isset($array['to']) && is_array($array['to'])) {
				$larray['event_description'] = 'Email sent to "' . safe_output($array['to']['to']) . '" from "' . $this->phpmailer->From . '" with subject "'.safe_output($array['subject']).'"';				
			}
			else {
				$larray['event_description'] = 'Email sent from "' . $this->phpmailer->From . '" with subject "'.safe_output($array['subject']).'"';
			}
			
			$larray['event_file'] = __FILE__;
			$larray['event_file_line'] = __LINE__;
			$larray['event_type'] = 'email_sent';
			$larray['event_source'] = 'mailer';
			$larray['event_version'] = '1';
			$larray['log_backtrace'] = false;	
			
			$log->add($larray);
			unset($larray);
			
			if (isset($array['cc']) && is_array($array['cc']) && !empty($array['cc'])) {
				$larray['event_description'] = 'Email was also CCed to: ';	
				
				foreach($array['cc'] as $cc_item) {	
					$larray['event_description'] .= safe_output($cc_item['to']) . ', ';
				}
				
				$larray['event_description'] = rtrim($larray['event_description'], ', ');
