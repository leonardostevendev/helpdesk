		...
		<div class="well well-sm">
			<form method="get" action="<?php echo safe_output($_SERVER['REQUEST_URI']); ?>">
				...
				<select name="event_source">
					...
					<option value="users"<?php if (isset($get_array['event_source']) && ($get_array['event_source'] == 'users')) echo ' selected="selected"'; ?>><?php echo safe_output($language->get('Users')); ?></option>
				</select>
				</p>
			...
			<table class="table table-striped table-bordered">
				...
				<tbody>
					...
					<tr <?php if ($i % 2 == 0 ) { echo 'class="switch-1"'; } else { echo 'class="switch-2"'; }; ?>>
						<td data-title="<?php echo safe_output($language->get('Added')); ?>" class="centre"><a href="<?php echo $config->get('address'); ?>/logs/view/<?php echo (int) $log_item['id']; ?>/"><?php echo safe_output(time_ago_in_words($log_item['event_date'])); ?> <?php echo safe_output($language->get('ago')); ?></a></td>
						<td data-title="<?php echo safe_output($language->get('User')); ?>" class="centre"><a href="<?php echo $config->get('address'); ?>/users/view/<?php echo (int) $log_item['user_id']; ?>/"><?php echo safe_output(ucwords($log_item['name'])); ?></a></td>
						<td data-title="<?php echo safe_output($language->get('IP Address')); ?>" class="centre"><?php echo safe_output($log_item['event_ip_address']); ?></td>
						<td data-title="<?php echo safe_output($language->get('Description')); ?>" class="centre"><?php echo html_output($log_item['event_description']); ?></td>
					</tr>
					<?php $i++; } ?>
				</tbody>
			</table>
